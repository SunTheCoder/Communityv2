...
import { Button, Input, Stack, Textarea, Card, createListCollection, Image, DrawerActionTrigger } from "@chakra-ui/react";
...
const AddResourceDrawer = ({ initialData = {}}) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
    ...
    // Prefill the form when `initialData` changes
    useEffect(() => {
        if (initialData) {
          setValue("resourceName", initialData.name || "");        
          setValue("description", initialData.description || "");
          setValue("streetAddress", initialData.location?.street_address || "");
          setValue("city", initialData.location?.city || "");
          setValue("state", [initialData.location?.state] || "");
          setValue("zipCode", initialData.location?.zip_code || "");
          setValue("resourceType", [initialData.type] || "");
          setValue("file", initialData.image || null);
  ...
  const onSubmit = async (data) => {
    ...
    const { latitude, longitude } = coordinates;
      ...
      // Update the resource request record after successfully adding the resource
    const { error: updateError } = await supabase
    .from("resource_requests")
    .update({ status: "approved" }) // Update the status to "approved"
    .eq("id", initialData.id); // Match the request by ID

    if (updateError) {
    console.error("Resource added but failed to update the resource request:", updateError.message);
    toaster.create({
    title: "Partial Success",
    description: "Resource was added, but the resource request status could not be updated.",
    type: "warning",
    });
    return;
    }
