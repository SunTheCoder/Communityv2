...
const AddResourceDrawer = ({ initialData = {} }) => {
  ...
  const onSubmit = async (data) => {
    ...
    const { latitude, longitude } = coordinates;
      ...
      // Delete the resource request record after successfully adding the resource
    const { error: deleteError } = await supabase
    .from("resource_requests")
    .delete()
    .eq("id", initialData.id); // Assuming `requestId` is passed as part of `data`

  if (deleteError) {
    console.error("Resource added but failed to delete the resource request:", deleteError.message);
    toaster.create({
      title: "Partial Success",
      description: "Resource was added, but the resource request could not be deleted.",
      type: "warning",
    });
    return;
  }
